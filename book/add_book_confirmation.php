...
include './DB.php';
	...
	if(isset($_POST['up'])){	//「登録」が押されたとき
		...
		if(!empty($value)){	//すでに同じISBNが入っていた場合
			...
			//本の情報をDBに入れる
			$flag = mysql_query("INSERT INTO  BOOK (ISBN, BOOK_NAME, AUTHOR_NAME, PUBLISHER, LINK, 	BOOK_NUMBER ,BOOK_PURCHASE ) VALUES ('$isbnText','$book_nameText','$authot_nameText', '$publisherText', '$linkText', '$book_numberText' ,'$book_purchaseText' ) ");			
			if($flag){
				//リクエストの処理
				reqest($isbnText);
				$res = mysql_query("SELECT * FROM BOOK ORDER BY ID DESC LIMIT 1");
				$value2=mysql_fetch_assoc($res);
				$bookId = $value2[ID];

				mysql_query("INSERT INTO CATEGORY (BOOK_ID , NAME ) VALUES ('$bookId','$category')");
	...
	<div class="sitebox">
		...
		<div class="main">
			...
			//全部typeがhiddenにして非表示で送るようにしている
				...
				echo '<p>●購入費</p>';
				echo '<p>'.$book_purchaseText.'</p>';
				echo '<input type="hidden" name="book_purchase" value="'.$book_purchaseText.'">';  //<!-- 冊数(book_numberにはいる) -->	
				?>
				<br> 
